...
def findMaxContour(contours):
    ...
    for i in range(len(contours)):  # Tüm konturlar arasında dolaşıyoruz.
        area_hand = cv2.contourArea(contours[i])  # Konturun alanını hesaplıyoruz.

        if max_area < area_hand:  # Eğer yeni alan daha büyükse, onu kaydediyoruz.
            max_area = area_hand
...
while True:
    ...
    if len(contours) > 0:  # Eğer en az bir kontur bulunursa
        ...
        c = math.sqrt((extTop[0] - extLeft[0]) ** 2 + (extTop[1] - extLeft[1]) ** 2)  # Sol-Üst
        b = math.sqrt((extRight[0] - extLeft[0]) ** 2 + (extRight[1] - extLeft[1]) ** 2)  # Sol-Sağ

        try:
            # Üçgenin açısını hesaplıyoruz (Kosünüs Teoremi kullanarak).
            ...
            # Eğer açı 70 dereceden büyükse sol üst köşeye mavi bir kare ekleyerek görsel uyarı veriyoruz.
            if angle_ab > 70:
                cv2.rectangle(frame, (0, 0), (100, 100), (255, 0, 0), -1)  # Mavi dikdörtgen çiziyoruz.
            else:
                pass
